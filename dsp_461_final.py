...
plt.title('Percentage of Diagnosis Group 1')
...
plt.title('Percentage of Diagnosis Group 2')
plt.ylabel('Percentage (%)')
plt.xlabel('Diagnosis Group')
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
...
plt.title('Percentage of Diagnosis Group 3')
plt.ylabel('Percentage (%)')
plt.xlabel('Diagnosis Group')
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
...
# Retrain LOS tree
mort_model = DecisionTreeRegressor(max_depth=4, random_state=42)
mort_model.fit(X, y)
...
def predict_length_of_stay(race, gender, age, diag_1_group, diag_2_group, diag_3_group, num_diagnoses):
    ...
    try:
        ...
        features = input_data.drop(['time_in_hospital'], axis=1)

        # Predict length of stay using your survived model
        length_of_stay_pred = tree_main.predict(features)[0]
